...
FileBrowser::FileBrowser(const fs::path &initial_path)
...
bool FileBrowser::loadDirectory(const fs::path &path)
{
  clearError();
...
void FileBrowser::pageDown(size_t page_size)
{
  if (!entries_.empty())
  {
    selected_index_ =
...
void FileBrowser::pageUp(size_t page_size)
{
  if (!entries_.empty())
  {
...
std::optional<fs::path> FileBrowser::getSelectedPath() const
{
...
bool FileBrowser::isSelectedDirectory() const
{
  if (selected_index_ >= entries_.size())
  {
    return false;
...
void FileBrowser::cycleSortMode()
{
  switch (sort_mode_)
  {
  case SortMode::TYPE:
...
void FileBrowser::updateScroll(size_t viewport_height)
{
...
size_t FileBrowser::getDirectoryCount() const
{
  return std::count_if(entries_.begin(), entries_.end(), [](const FileEntry &e)
                       { return e.is_directory && e.name != ".."; });
}

size_t FileBrowser::getFileCount() const
{
...
FileBrowser::createEntry(const fs::directory_entry &entry)
{
...
bool FileBrowser::isExecutable(const fs::path &path) const
{
#ifdef _WIN32
